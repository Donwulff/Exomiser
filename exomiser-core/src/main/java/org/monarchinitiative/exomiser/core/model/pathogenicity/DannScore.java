...
/**
 * DANN info - see {@link https://doi.org/10.1093/bioinformatics/btu703}
 * 
 * @author 
 */
public class DannScore extends ScaledPathogenicityScore {

    /**
     * Creates a {@link DannScore} from the input PHRED scaled score. *IMPORTANT* this method will rescale the input
     ...
     */
    public static DannScore of(float phredScaledScore) {
//        float score = 1 - (float) Math.pow(10, -(phredScaledScore / 10));
	float score = phredScaledScore;
        return new DannScore(phredScaledScore, score);
    }
        
    private DannScore(float rawScore, float scaledScore) {
        super(PathogenicitySource.DANN, rawScore, scaledScore);
    }
